...
#Read in image
img = imread("input_images/aging2_transformed_cropped.tif")
...
img_adj = adjust_gamma(img, 0.7)
img_adj[img_cp == 1] = img_adj.max()
imwrite("processed_images_transformed/aging2/test.tif", img_adj)

#Apply smoothing
img_smooth = edge_preserving_smoothing_3d(img_adj, numberOfIterations=10)
imwrite("processed_images_transformed/aging2/img_smooth.tif", img_smooth)

#Segmentation
img_seg = filament_2d_wrapper(img_smooth, seg_param)
imwrite("processed_images_transformed/aging2/seg.tif", img_seg)
...
imwrite("processed_images_transformed/aging2/aging2_segmentation_frompipeline.tif", img_fin)
...
#Save mask
imwrite("processed_images_transformed/aging2/aging2_mask.tif", mask_noholes)
...
imwrite("processed_images_transformed/aging2/aging2_skel_raw.tif", skel)
segm_masked = segm_masked.astype("bool")
imwrite("processed_images_transformed/aging2/aging2_segmentation.tif", segm_masked)


#CREATE NEW SKELETON BASED ON UPDATED SEGMENTATION

#Load
segm_fixed = imread("processed_images_transformed/aging2/aging2_segmentation.tif")

skel = skeletonize(segm_fixed)
skel = skel.astype("bool")

imwrite("processed_images_transformed/aging2/aging2_skeleton.tif", skel)
