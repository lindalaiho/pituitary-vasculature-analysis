...
#Read in image
img = imread("input_images/aging5_transformed_cropped.tif")
...
img_adj = adjust_gamma(img, 0.2)
img_adj[img_cp == 1] = img_adj.max()

#Apply smoothing
img_smooth = edge_preserving_smoothing_3d(img_adj, numberOfIterations=10)
imwrite("processed_images_transformed/aging5/img_smooth.tif", img_smooth)
...
imwrite("processed_images_transformed/aging5/aging5_segmentation_frompipeline.tif", img_fin)
...
img_rescaled = rescale_intensity(img, in_range=(0, 450))
...
mask_noholes = hole_filling(mask_closed2, hole_min=1, hole_max=6000, fill_2d=True)

#Save mask
imwrite("processed_images_transformed/aging5/aging5_mask.tif", mask_noholes)
...
imwrite("processed_images_transformed/aging5/aging5_skel_raw.tif", skel)
segm_masked = segm_masked.astype("bool")
imwrite("processed_images_transformed/aging5/aging5_segmentation.tif", segm_masked)


#CREATE NEW SKELETON BASED ON UPDATED SEGMENTATION

#Load
segm_fixed = imread("processed_images_transformed/aging5/aging5_segmentation.tif")

skel = skeletonize(segm_fixed)
skel = skel.astype("bool")

imwrite("processed_images_transformed/aging5/aging5_skeleton.tif", skel)