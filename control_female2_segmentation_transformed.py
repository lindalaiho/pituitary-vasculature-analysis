...
#Read in image
img = imread("input_images/control_female2_transformed_cropped.tif")
...
imwrite("processed_images_transformed/control_female2/img_smooth.tif", img_smooth)
...
imwrite("processed_images_transformed/control_female2/control_female2_segmentation_frompipeline.tif", img_fin)
...
img_rescaled = rescale_intensity(img, in_range=(69, 3700))

#Binarise at appropriate threshold
mask = img_rescaled > 17000
...
mask_noholes = hole_filling(mask_closed2, hole_min=1, hole_max=2500, fill_2d=True)

#Save mask
imwrite("processed_images_transformed/control_female2/control_female2_mask.tif", mask_noholes)
...
imwrite("processed_images_transformed/control_female2/control_female2_skel_raw.tif", skel)
segm_masked = segm_masked.astype("bool")
imwrite("processed_images_transformed/control_female2/control_female2_segmentation.tif", segm_masked)


#CREATE NEW SKELETON BASED ON UPDATED SEGMENTATION

#Load
segm_fixed = imread("processed_images_transformed/control_female2/control_female2_segmentation.tif")

skel = skeletonize(segm_fixed)
skel = skel.astype("bool")

imwrite("processed_images_transformed/control_female2/control_female2_skeleton.tif", skel)
