...
#Read in image
img = imread("input_images/lactating1_transformed_cropped.tif")
...
img_adj = adjust_gamma(img, 0.3)
img_adj[img_cp == 1] = img_adj.max()

#Apply smoothing
img_smooth = edge_preserving_smoothing_3d(img_adj, numberOfIterations=10)
imwrite("processed_images_transformed/lactating1/img_smooth.tif", img_smooth)
...
imwrite("processed_images_transformed/lactating1/lactating1_segmentation_frompipeline.tif", img_fin)
...
img_rescaled = rescale_intensity(img, in_range=(42, 511))

#Binarise at appropriate threshold
mask = img_rescaled > 44000
...
#Save mask
imwrite("processed_images_transformed/lactating1/lactating1_mask.tif", mask_noholes)
...
imwrite("processed_images_transformed/lactating1/lactating1_skel_raw.tif", skel)
segm_masked = segm_masked.astype("bool")
imwrite("processed_images_transformed/lactating1/lactating1_segmentation.tif", segm_masked)


#CREATE NEW SKELETON BASED ON UPDATED SEGMENTATION

#Load
segm_fixed = imread("processed_images_transformed/lactating1/lactating1_segmentation.tif")

skel = skeletonize(segm_fixed)
skel = skel.astype("bool")

imwrite("processed_images_transformed/lactating1/lactating1_skeleton.tif", skel)
