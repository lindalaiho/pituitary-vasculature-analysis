...
#Read in image
img = imread("input_images/stress1_transformed_cropped.tif")
...
img_adj = adjust_gamma(img, 0.5)
img_adj[img_cp == 1] = img_adj.max()

#Apply smoothing
img_smooth = edge_preserving_smoothing_3d(img_adj, numberOfIterations=10)
imwrite("processed_images_transformed/stress1/img_smooth.tif", img_smooth)
...
imwrite("processed_images_transformed/stress1/stress1_segmentation_frompipeline.tif", img_fin)
...
img_rescaled = rescale_intensity(img, in_range=(0, 250))

#Binarise at appropriate threshold
mask = img_rescaled > 65000
...
#Save mask
imwrite("processed_images_transformed/stress1/stress1_mask.tif", mask_noholes)
...
imwrite("processed_images_transformed/stress1/stress1_skel_raw.tif", skel)
segm_masked = segm_masked.astype("bool")
imwrite("processed_images_transformed/stress1/stress1_segmentation.tif", segm_masked)


#CREATE NEW SKELETON BASED ON UPDATED SEGMENTATION

#Load
segm_fixed = imread("processed_images_transformed/stress1/stress1_segmentation.tif")

skel = skeletonize(segm_fixed)
skel = skel.astype("bool")

imwrite("processed_images_transformed/stress1/stress1_skeleton.tif", skel)
