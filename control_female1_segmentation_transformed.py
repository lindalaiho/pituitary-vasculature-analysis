...
#Read in image
img = imread("input_images/control_female1_transformed_cropped.tif")
...
img_adj = adjust_gamma(img, 0.45)
img_adj[img_cp == 1] = img_adj.max()

#Apply smoothing
img_smooth = edge_preserving_smoothing_3d(img_adj, numberOfIterations=10)
imwrite("processed_images_transformed/control_female1/img_smooth.tif", img_smooth)
...
imwrite("processed_images_transformed/control_female1/control_female1_segmentation_frompipeline.tif", img_fin)
...
img_rescaled = rescale_intensity(img, in_range=(0, 1300))
...
mask_noholes = hole_filling(mask_closed2, hole_min=1, hole_max=4000, fill_2d=True)

#Save mask
imwrite("processed_images_transformed/control_female1/control_female1_mask.tif", mask_noholes)
...
imwrite("processed_images_transformed/control_female1/control_female1_skel_raw.tif", skel)
segm_masked = segm_masked.astype("bool")
imwrite("processed_images_transformed/control_female1/control_female1_segmentation.tif", segm_masked)


#CREATE NEW SKELETON BASED ON UPDATED SEGMENTATION

#Load
segm_fixed = imread("processed_images_transformed/control_female1/control_female1_segmentation.tif")

skel = skeletonize(segm_fixed)
skel = skel.astype("bool")

imwrite("processed_images_transformed/control_female1/control_female1_skeleton.tif", skel)
